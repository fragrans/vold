...
{
  LOG_VOL("handle_usb_event");
  return 0;
  if (event->action == action_add) {
    char *type;
    char *path;
    type = get_uevent_param(event, "DEVTYPE");
    if (0 == strcmp(type, "usb_device")) {
      sleep(10);
      ...
      for (f=fs; *f != NULL; f++) {
        ...
        mkdir("/sdcard/sdb", 0755);
        rc = mount("/dev/block/sdb", "/sdcard/sdb", *fs, flags, NULL);
        LOG_VOL("RW rc = %d; errno = %d", rc, errno);
          
        if (rc && errno == EROFS) {

          flags |= MS_RDONLY;
          rc = mount("/dev/block/sdb", "/sdcard/sdb", *fs, flags, NULL);
          LOG_VOL("RO rc = %d; errno = %d", rc, errno);
        }
      }
        send_msg_with_data(VOLD_EVT_MOUNTED, "/dev/block/sdb");
    }
  } else if (event->action == action_remove) {
    //add operation
  } else{
